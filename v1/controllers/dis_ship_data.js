    ...
    await call_back.call(email,(err,data)=>{
        if (err) {
            ...
            for (var i = 0;i<len;i++){
                ...
                if (id == data[i]._id){
                    var del_data = data[i].scan
                    //console.log(del_data)
                }
                
            }
            res.render('v1/public/das_bord.ejs',{DEL:cou_1,INT:cou_2,OOD:cou_3,DEX:cou_4,NFI:cou_5,data:data,status:req.cookies.status,del_data:del_data})
        }
    })
}